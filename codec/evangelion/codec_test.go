...
func TestServerEncodeClientDecode(t *testing.T) {
	response := &Response{
	...
	// protobuf message has some internal fields,
	// it will change,
	// so we cannot use reflect.DeepEqual(response, drsp).
	drsp := rsp.(*Response)
...
func TestClientEncodeServerDecode(t *testing.T) {
	request := &Request{
